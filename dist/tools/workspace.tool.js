...
export class WorkspaceTool extends BaseTool {
    ...
    tools = [
        ...
        },
        {
            ...
            schema: {
                ...
                slug: z
                    ...
                    .regex(/^[A-Za-z0-9_]+$/)
                    .optional()
                    .describe("A unique identifier for the custom field (letters, numbers, and underscores only)."),
            ...
            handler: async (args, ctx) => {
                const { name, slug, description, dataType, allowedValues } = args;
                try {
                    const sdkClient = createSdkClient(this.apiBaseUrl, this.mode, ctx);
        ...
        },
        {
            ...
            handler: async (args, ctx) => {
                const { channelId, limit = 10, cursorId, } = args;
                try {
                    const sdkClient = createSdkClient(this.apiBaseUrl, this.mode, ctx);
        ...
        },
        {
            ...
            handler: async (args, ctx) => {
                const { name, description, colorCode, emoji } = args;
                try {
                    const sdkClient = createSdkClient(this.apiBaseUrl, this.mode, ctx);
        ...
        },
        {
            ...
            handler: async (args, ctx) => {
                const { currentName, name, colorCode } = args;
                try {
                    const sdkClient = createSdkClient(this.apiBaseUrl, this.mode, ctx);
